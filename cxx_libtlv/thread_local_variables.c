...
static void tlv_set_key_for_image(const struct mach_header *mh,
                                  pthread_key_t key) {
...
tlv_allocate_and_initialize_for_key(pthread_key_t key) {
  ...
  for (uint32_t i = 0; i < cmd_count; ++i) {
    if (cmd->cmd == LC_SEGMENT_COMMAND) {
      ...
      for (const macho_section *sect = sectionsStart; sect < sectionsEnd;
           ++sect) {
        ...
        case S_THREAD_LOCAL_REGULAR:
          if (start == NULL) {
            // first of N contiguous TLV template sections, record as if this
            // was only section
            start = (uint8_t *)(sect->addr + slide);
            size = sect->size;
          } else {
            // non-first of N contiguous TLV template sections, accumlate values
            const uint8_t *newEnd =
                (uint8_t *)(sect->addr + slide + sect->size);
            size = newEnd - start;
          }
          break;
        }
...
static void tlv_free(void *storage) { free(storage); }
      ...
      for (const macho_section *sect = sectionsStart; sect < sectionsEnd;
        ...
        if ((sect->flags & SECTION_TYPE) == S_THREAD_LOCAL_VARIABLES) {
          if (sect->size != 0) {
            ...
            TLVDescriptor *end =
                (TLVDescriptor *)(sect->addr + sect->size + slide);
...
static void tlv_load_notification(const struct mach_header *mh,
                                  intptr_t slide) {
  (void)slide;

  // This is called on all images, even those without TLVs. So we want this to
  // be fast. The linker sets MH_HAS_TLV_DESCRIPTORS so we don't have to search
  // images just to find the don't have TLVs.
...
void _tlv_atexit(TermFunc func, void *objAddr) {
  // NOTE: this does not need locks because it only operates on current thread
  // data
  struct TLVTerminatorList *list =
      (struct TLVTerminatorList *)pthread_getspecific(tlv_terminators_key);

  if (list == NULL) {
    // handle first allocation
    list = (struct TLVTerminatorList *)malloc(
        offsetof(struct TLVTerminatorList, entries[1]));
    ...
    if (list->useCount == list->allocCount) {
      ...
      struct TLVTerminatorList *newlist =
          (struct TLVTerminatorList *)malloc(newAllocSize);
...
static void tlv_finalize_list(struct TLVTerminatorList *list) {
  ...
  for (uint32_t i = list->useCount; i > 0; --i) {
    ...
    if (newlist != NULL) {
      // Set the list to NULL so that if yet another tlv is registered, we put
      // it in a new list
...
static void tlv_finalize(void *storage) {
  ...
  // also set it to NULL before calling us.  If new thread locals are added to
  // our tlv_terminators_key, then they will be on a new list, but the list we
  // have here is one we own and need to destroy it
...
__attribute__((visibility("hidden"))) void tlv_initializer(void) {
...
// linked images with TLV have references to this symbol, but it is never used
// at runtime
